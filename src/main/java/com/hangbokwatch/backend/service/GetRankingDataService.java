...
public class GetRankingDataService {
    ...
    public List<PlayerRankingDto> getTankRatingRankingData(Map<String, Object> sessionItems) {
        ...
        playerRankingDtoList = parseDomainToDTO(rankingDataList);
    ...
    public List<PlayerListDto> getTankRankergData(Map<String, Object> sessionItems) {
        String sessionBattleTag = (String) sessionItems.get("sessionBattleTag");
        ...
        List<Player> rankerDataList = playerRepository.findAllByIsPublicOrderByTankRatingPointDesc("Y", page).getContent();
    ...
    public List<PlayerListDto> getDealRankerData(Map<String, Object> sessionItems) {
        String sessionBattleTag = (String) sessionItems.get("sessionBattleTag");
        List<PlayerListDto> playerListDtoList = new ArrayList<>();

        log.info("{} >>>>>>>> getDealRankerData 호출 | 딜러 랭커 데이터 추출 시작 ", sessionBattleTag);
        Pageable page = PageRequest.of(1,25);
        List<Player> rankerDataList = playerRepository.findAllByIsPublicOrderByDealRatingPointDesc("Y", page).getContent();
        playerListDtoList = parseDomainToDTO2(rankerDataList);
    ...
    public List<PlayerListDto> getHealRankerData(Map<String, Object> sessionItems) {
        String sessionBattleTag = (String) sessionItems.get("sessionBattleTag");
        List<PlayerListDto> playerListDtoList = new ArrayList<>();

        log.info("{} >>>>>>>> getHealRankerData 호출 | 힐러 랭커 데이터 추출 시작 ", sessionBattleTag);
        Pageable page = PageRequest.of(1,25);
        List<Player> rankerDataList = playerRepository.findAllByIsPublicOrderByHealRatingPointDesc("Y", page).getContent();
        playerListDtoList = parseDomainToDTO2(rankerDataList);
    ...
    public List<PlayerListDto> getTotalAvgRankerData(Map<String, Object> sessionItems) {
        String sessionBattleTag = (String) sessionItems.get("sessionBattleTag");
        List<PlayerListDto> playerListDtoList = new ArrayList<>();

        log.info("{} >>>>>>>> getHealRankerData 호출 | 힐러 랭커 데이터 추출 시작 ", sessionBattleTag);
        Pageable page = PageRequest.of(1,25);
        List<Player> rankerDataList = playerRepository.findAllByIsPublicOrderByTotalAvgRatingPointDesc("Y", page).getContent();
        playerListDtoList = parseDomainToDTO2(rankerDataList);
        log.info("{} >>>>>>>> getHealRankerData 종료 | 힐러 랭커 데이터 추출 완료 ", sessionBattleTag);
    ...
    public List<PlayerRankingDto> parseDomainToDTO(List<PlayerForRanking> rankingDataList) {
        List<PlayerRankingDto> playerRankingDtoList = new ArrayList<>();
        for (PlayerForRanking data : rankingDataList) {
            Player player = playerRepository.findPlayerById(data.getId());

            PlayerRankingDto playerRankingDto = new PlayerRankingDto(data.getId(), player.getBattleTag(), player.getPlayerName(), player.getPortrait(),
                    data.getTankRatingPoint(), data.getDealRatingPoint(), data.getHealRatingPoint(), data.getWinGame(),data.getLoseGame(),
                    data.getDrawGame(), changeTimeToString(data.getPlayTime()), changeTimeToString(data.getSpentOnFire()), data.getEnvKill());
